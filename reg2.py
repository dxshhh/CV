...
def validate(net, dataset, num):
    ...
    for idx in sel:
        ...
        if true_match == 1:
            ...
            pose_err = float(torch.acos(cs[0] * cs[1]))
            acc_pose += pose_err
            print(pose_err * 180 / math.pi)
...
def main():
    ...
    print('acc_match:%.2f, acc_pose:%.2f' % validate(net, testdataset, min(1000, len(testdataset))))
